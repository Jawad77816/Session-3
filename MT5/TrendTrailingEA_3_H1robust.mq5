...
input double             InpADXThreshold   = 22.0;          // Minimum ADX to call it a trend

input group "=== Entry ==="
input double             InpLots           = 0.01;           // Lot size
input long               InpMagic          = 990133;         // Magic number

input group "=== Pivot-Rejection Entry Trigger ==="
input ENUM_TIMEFRAMES    InpSignalTF       = PERIOD_M5     ; // Timeframe whose candles are checked for rejections
...
int TrendDirection()
  {
   // ADX momentum gate: in chop (low ADX) report FLAT so no new trades fire,
   // while remembering the last confirmed direction for when momentum returns.
   if(InpUseADX && adxHandle != INVALID_HANDLE)
     {
      double adx[];
      if(CopyBuffer(adxHandle, 0, 1, 1, adx) >= 1 && adx[0] < InpADXThreshold)
         return 0;
     }

   // Confirmed direction: the last N closed HTF bars must all clear the MA by
   // the deadband on the SAME side before the trend state is allowed to flip.
   int need = InpUseConfirmBars ? (int)MathMax(1, InpConfirmBars) : 1;
   double ma[];
   if(CopyBuffer(maHandle, 0, 1, need, ma) < need)
      return g_trendState;
   ...
   if(cd != 0)
      g_trendState = cd;        // flip only when all N bars confirm past the deadband
   return g_trendState;         // otherwise hold the previous confirmed state
  }
...
void DashPanel()
  {
   ...
   ObjectSetInteger(0, obj, OBJPROP_YSIZE, 340);
  }
...
void UpdateDashboard()
  {
   ...
   DashLabel(r++, "TrendTrailing [3:H1+rob] " + _Symbol, cHead);
   ...
   if(InpUseADX && adxHandle != INVALID_HANDLE)
     {
      double adxv[]; double av = 0.0;
      if(CopyBuffer(adxHandle, 0, 1, 1, adxv) >= 1) av = adxv[0];
      DashLabel(r++, StringFormat("ADX(%d): %.1f  min %.0f%s", InpADXPeriod, av, InpADXThreshold,
                (av < InpADXThreshold ? "  CHOP" : "")),
                (av < InpADXThreshold) ? clrOrange : cText);
     }
