...
//|  the Strategy Tester.                                            |
//|                                                                  |
//|  BUY  : red -> green -> green, middle only needs the dip (lowest   |
//|         low with wick beyond neighbours), 3rd body closes above    |
//|         candle 1's high. No body-inside-first, no hammer, no RSI.  |
//|  SELL : mirror image.                                            |
//|                                                                  |
//|  Auto SL/TP: SL at the middle candle's extreme, TP = ratio x risk|
//+------------------------------------------------------------------+
#property copyright "Three-Candle Reversal EA (No-Pin)"
#property version   "1.00"
#property strict
#property description "Self-contained three-candle reversal (No-Pin build) for XAUUSD M1/M5."
#property description "Middle candle only needs the dip/spike; no body-inside-first, hammer or RSI."
...
input long     InpMagicNumber    = 20250812;    // Magic number
...
string   g_lastCheck   = "-";
#define  DASH_PREFIX     "TCREANOPIN_DASH_"
...
void DrawDashboard()
  {
   ...
   DashLabel("l0", "3-Candle Reversal EA (No-Pin)", x, y + (n++) * lh, clrGold, 10);
   ...
   DashLabel("l3", "Filters : Dip=Y Trend=" + (InpUseTrendFilter?"Y":"N")
...
int OnInit()
  {
   ...
   Print("ThreeCandleReversalEA (No-Pin) v1.00 started on ", _Symbol, " ", TfToStr((ENUM_TIMEFRAMES)_Period));
